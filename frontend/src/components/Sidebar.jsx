...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-br from-[#242124]/30 to-[#0000000/30] border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      {/* Logo */}
      ...
      <div className='flex flex-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md'>
      <img src={assets.search_icon} alt="" className='w-4 not-dark:invert' />
      <input onChange={(e)=>setSearch(e.target.value)} value={search} type="text" placeholder='Search Conversations' className='text-x5 placeholder:text-gray-400 outline-none'/>
      </div>

      {/* Recent Chats */}
      {chats.lengthh > 0 && <p className='mt-4 text-sm'>Recent Chats</p>}
      ...
      <div onClick={()=> {navigate('/credits'); setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
          ...
          <p className='text-x5 text-gray-400'>Purchase credits to use quickgpt</p>
        </div>
      </div>
